...
from cosipy.cpkernel.node_agetracking import Node
...
class Grid:
    ...
    def update_node_ages(self):
        """Update the creation and burial ages of all nodes.

        - Increment both ages by 1.
        - Reset burial_age of top node to 0.
        """
        for i in range(self.number_nodes):
            current_node = self.grid[i]
            current_node.set_layer_creation_age(current_node.get_layer_creation_age() + 1)
            current_node.set_layer_burial_age(current_node.get_layer_burial_age() + 1)

        # Reset burial age for the topmost node
        if self.number_nodes > 0:
            self.grid[0].set_layer_burial_age(0)

    def get_node_creation_age(self, idx: int):
        return self.grid[idx].get_layer_creation_age()

    def get_node_burial_age(self, idx: int):
        return self.grid[idx].get_layer_burial_age()
    
    def get_creation_age(self) -> list:
        """Get the temperature profile."""
        return [
            self.grid[idx].get_layer_creation_age()
            for idx in range(self.number_nodes)
        ]
    
    def get_burial_age(self) -> list:
        """Get the temperature profile."""
        return [
            self.grid[idx].get_layer_burial_age()
            for idx in range(self.number_nodes)
        ]
    ...
    def add_fresh_snow(
        ...
        # Add new node with age tracking
        new_node = Node(height, density, temperature, liquid_water_content, None, 0.0)
        new_node.set_layer_creation_age(0)
        new_node.set_layer_burial_age(0)

        self.grid.insert(0, new_node)
        self.number_nodes += 1
    ...
    def init_grid(self):
        for idxNode in range(self.number_nodes):
            ...
            node.set_layer_creation_age(0)
            node.set_layer_burial_age(0)
    ...
    def merge_nodes(self, idx: int):
        ...
        age_creation = max(
            self.grid[idx].get_layer_creation_age(),
            self.grid[idx + 1].get_layer_creation_age()
        )
        age_burial = min(
            self.grid[idx].get_layer_burial_age(),
            self.grid[idx + 1].get_layer_burial_age()
        )
        ...
        self.grid[idx].set_layer_creation_age(age_creation)
        self.grid[idx].set_layer_burial_age(age_burial)
    ...
    def update_grid(self):
        ...
        if merge_deepest_layer:
            while self.get_number_layers()>= max_layers-5:
                print('merging deepest layer')
